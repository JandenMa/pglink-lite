...
const ModelImpl = (connection, globalAutoSetTimeFields) =>
  ...
  class ModelBase {
     ...
     * @param {function} callback Function to be run before comitting the database operation
     */
    async multiInsert(items, callback) {
      try {
        const datas = [];
        items.forEach(item => {
          datas.push({
        ...
        const res = await this.dataAccess.MultiInsertExecutor(datas, callback);
        return res ? this.decodeToEnum(res) : [];
      } catch (e) {
        throw e;
      }
    ...
    async multiUpdateWithConditions(
      ...
      try {
        ...
        const res = await this.dataAccess.MultiUpdateExecutor(datas, callback);
        return res ? this.decodeToEnum(res) : [];
      } catch (e) {
        throw e;
      }
     ...
     * @param {function} callback Function to be run before comitting the database operation
     */
    async deleteByConditions(whereClause, callback) {
      try {

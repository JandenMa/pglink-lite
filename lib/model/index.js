...
const ModelImpl = connection =>
  ...
  class ModelBase {
     ...
     * @param {string} whereClause e.g. "employeeId" = '123'
     */
    async updateByConditions(params, whereClause) {
      try {
    ...
    /**
     *
     * @param {*} items the array of data to be inserted into table
     */
     ...
     *
     * @param {*} items the array of data to be inserted into table
     * @param {*} whereClause needs to dynamically pass in logic not sure how
     *  to do this haha.
     * notes: currently only works with where clause of null
     */

    async multiUpdateWithConditions(items, whereClause = null) {
      try {
        const datas = []
        items.forEach(item => {
          datas.push({

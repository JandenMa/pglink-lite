...
const ModelImpl = (connection, globalAutoSetTimeFields) =>
	 ...
	 */
	class ModelBase {
		static dataAccess = new DataAccess(connection)
		/**
		 * @constructor
		 * @description A base class for other classes to operate CRUD
        ...
        let data = this.decodeToEnum(res)
        
        if(!(data isinstanceof Array)) data = [data]
				...
				const instanceArray = data.map(d => {
					return new this(d)
				})
		 ...
		 * @param {object} data data to authorize against
		 */
		...
		// This needs to be accessible to both the Class and instances of it, so wrapping the constructor authorize in an instance authorize makes sense, also necessary because the static authorize does not have access to instance data
		authorize(authType, data) {
			return this.constructor.authorize(authType, data || this.data)
		}
		...
		static async findByPk(pk, selectFields = '*') {
			try {
				...
				const data = this.decodeToEnum(res[0])
		...
		async insertOne(params, callback) {
          ...
          callback
				...
				return this.decodeToEnum(res[0])
			} catch (e) {
				throw e
			}
		}
		 ...
		 */
		async updateByPk(params, callback, autoSetTimeFields = this.autoSetTimeFields) {
			try {
		...
		async save(authType, callback) {
			try {
				...
				switch (auth) {
					case 'insert':
						this.data = await this.insertOne(this.data, callback)
						break
					case 'update':
						this.data = await this.updateByPk(this.data, callback)
						break
					default:
		 ...
		 */
		async saveWithPeers(peersArray, authTypes, callback) {
			try {
				...
				const params = await Promise.all(
					peersArray.map(async (peer, index) => {
						...
						return {
							...
							pkName: peer.pkName,
							autoSetTimeFields: peer.autoSetTimeFields
						}
					})
				)

				const res = await this.dataAccess.multiInsertOrUpdateExecutor(
					params,
					callback
				)
		...
		encodeFromEnum(args) {
			return this.constructor.encodeFromEnum(args)
		}
		...
		decodeToEnum(args) {
			return this.constructor.decodeToEnum(args)
		}
	}

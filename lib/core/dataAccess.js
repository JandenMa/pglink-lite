...
class DataAccess {
	...
	async Transaction(args, transaction) {
      ...
      ************************************************************************** */
			...
			resArray = resArray.map(res =>
				res.rows.length > 1 ? res.rows : res.rows[0]
			)
	...
	async multiInsertOrUpdateExecutor(items, callback) {
		const sqls = []

		items.forEach(item => {
			const { data, tableName, pkName, autoSetTimeFields } = item
			...
			if (data[pkName]) {
				...
				const sql = this.GenerateInsertSQL(data, tableName)

...
class DataAccess {
  ...
  async MultiUpdateExecutor({items, forceFlat, callback, client}) {
    ...
    const res = await this.Transaction({ params: sqls, forceFlat, client }, callback)
    return res
  }
   ...
   * @returns {Object} the response from postgres
   */
  async DeleteExecutor({tableName, whereClause, returnSingleRecord, callback, client}) {
    ...
    const res = await this.Transaction(
      {
        params: [{ sql, alias: tableName}],  forceFlat: true, returnSingleRecord, client 
      },
      callback
    )
    return res

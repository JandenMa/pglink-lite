...
class DataAccess {
  ...
  GenerateUpdateSQL(params = {}, tableName, whereClause, pkName = 'id') {
    ...
    if (whereClause) {
      ...
      where = where.concat(` AND "${pkName}" = '${params[pkName] || null}'`)
    }
    let setSql = ''
    paramsArray.forEach((key, index) => {
  ...
  GenerateMultiUpdateSQL(
    ...
    params.forEach((p, index) => {
      let tmp = ''
      const updateFieldsTmp = [...updateFields, `"${pkName}"`]
    ...
    AS tmp(${cstr}) WHERE tmp."${pkName}" = "${tableName}"."${pkName}" RETURNING *;`
    return { sql, paramsArray }
  }
   ...
   * @returns {Object} the response from postgres
   */
  async SingleQueryExecutor(tableName, whereClause, selectFields = '*') {

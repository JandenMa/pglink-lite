...
class DataAccess {
   ...
   */
  async InsertExecutor({ params, tableName, callback, client }) {
    const sql = this.GenerateInsertSQL(params, tableName)
    const res = await this.Transaction(
      { params: [sql], returnSingleRecord: true, client },
      callback
    )
  ...
  async MultiInsertToOneTableExecutor({
    ...
    const res = await this.Transaction(
      { params: [sql], forceFlat: true, client },
      callback
    )
    return res
  }
   ...
   */
  async MultiInsertExecutor({ items, forceFlat, callback, client }) {
    const sqls = []
    items.forEach((item) => {
  ...
  async UpdateExecutor({
    ...
    const res = await this.Transaction({ params: [sql], client }, callback)
    return res
  }
   ...
   */
  async MultiUpdateExecutor({ items, forceFlat, callback, client }) {
    const sqls = []
    await Promise.all(
  ...
  async SingleQueryExecutor(args) {
    ...
    // actually return without alias, if need set returnWithAlias is true
    const res = await this.Transaction(
      { params: [{ sql, alias: tableName }], returnSingleRecord, client },
      callback
    )
    return res
  }

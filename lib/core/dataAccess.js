 ...
 */
/*
 *Standardize every output to be object or array of objects
...
class DataAccess {
   ...
   * @param {{params: Array<{sql: string,replacements: Array<any>,alias: string, flatten:bool}>,returnWithAlias: boolean}} args includes sqls, their params and alias name.
  ...
  async Transaction(args, transaction) {
    ...
    const { params, returnWithAlias = false, flatten = false } = args; // v0.1.9
    ...
    try {
      ...
      ************************************************************************** */
      ...
      let flatResult = result;

      while (
        flatResult instanceof Array &&
        ((flatten && flatResult.length === 1) ||
          (!flatten &&
            flatResult.length === 1 &&
            flatResult[0] instanceof Array))
      ) {
        flatResult = flatResult[0];
      }

      if (transaction) await transaction(flatResult);
   ...
   */
  async MultiInsertExecutor(items, callback) {
    const sqls = [];
    items.forEach(item => {
   ...
   */
  async MultiUpdateExecutor(items, callback) {
    const sqls = [];
    items.forEach(item => {
      const {
        params,
    ...
    const res = await this.Transaction({ params: sqls }, callback);
    return res;
  }
   ...
   * @returns {Object} the response from postgres
   */
  async DeleteExecutor(tableName, whereClause, callback) {
   ...
   * @param {tableName:string,whereClause:string,selectFields:string,sortBy:string,limit:number,offset:number, callback:function, flatten:bool} args
   * @returns {Object} the response from postgres
   */
  async SingleQueryExecutor(args) {
    ...
    // actually return without alias, if need set returnWithAlias is true
    const res = await this.Transaction(
      { params: [{ sql, alias: tableName }], flatten },
      callback
    );
    return res;
  }
